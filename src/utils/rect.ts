...
export class Rect {
  ...
  static fromDOM(dom: HTMLElement, isPseudoElem = false): Rect {
    if (isPseudoElem) {
      let {top, left, width, height} = dom.getBoundingClientRect()
      const pseudoWidth = parseFloat(window.getComputedStyle(dom, ':before').getPropertyValue('width'))
      const pseudoHeight = parseFloat(window.getComputedStyle(dom, ':before').getPropertyValue('height'))
      left -= ((pseudoWidth - width) / 2)
      top -= ((pseudoHeight - height) / 2)
      width = pseudoWidth
      height = pseudoHeight
